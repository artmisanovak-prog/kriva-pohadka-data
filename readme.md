...
- Mezera pro trénink je v ilustrací třetí autonomní složka. Stejně jako obraz - na kterém je vždy dívka představující medium řeči a zvířetem, které je nosičem její poruchy. A text - který je ručně kreslený a není popisem obrazu ani obraz ilustrací textu. V napětí mezi jejich sdělením vzniká mezera jako prostor pro vznik nových významů. Mezera v tomto systemu není novinka ve smyslu nějakého výmyslu, ale naopak je to reálně existující věc, kterou všichni známe, jen ji explicitně nepoužíváme jako něco, hodného vlastní práce s ní - to také systém netvrdí. Ve skutečnosti je to spíš prostor, kde význam teprve vzniká, něco předvýznamového, co ale význam vůbec umožňuje rozpoznávat a peracovat s ním. Není to prázdno, kde něco chybí, je to místo, kde můžou vznikat nove významy, kde se dají vidět souvislosti a vztahy a abstrahovat, kde se teprve rozhoduje, který význam se uskuteční a jak.
A system Chceš myš? s tímto prostorem explicitně pracuje a jako fyzický objekt existuje jako `gap.json`.
- `doma/poruchy/` – poruchy jsou věci, které běžně označujeme jako chyby. Jejich nosičem je zvíře a tím se z chyby stává entita, dostává fyzičnost. Stejně jako jazyk samotný přestává být jen abstraktní, ale dá se například krkat, věšet, může být postavou, může mít názor. Porucha nebo teď už ne porucha, ale jev nebo fenomén existuje na ilustraci i v křivé pohadce kde jazyk je performativní a forma textu zároveň provádí to o čem text právě mluví. Systém netvrdí, že není rozdíl mezi dobře a špatně ani že chyby neexistují. Jen přistupuje úplně jinak k celému pojetí této problematiky. Chyba zde není k opravě, naopak obsahuje často nejdůležitější informace.
 Jazyk není popis, ale materiál.
- Celý tento svět, systemu Chceš myš? Jsou vlastně záznamy myši, která není neobyčejná, jen si všímá věcí, které se běžně přehlížejí jako například, že se ve vzduchu občas zadrhne věta nebo páva přepočítávající své oči, když se nikdo nedívá. A myš je nesoudí, nehodnotí jen je zapisuje. Jenomže ne na papír, ale právě do těch mezer, do koutů, do skulin mezi slovy, kde to obvykle nikdo nečte. Myš je alterego autorky, zároveň postava v křivé pohádce, zároveň záznamník reality složené z absurdních pravidel - na kterých je nejabsurdnější to, že na ně obyčejná logika nestačí. Zároveň pozornost k detailům. Zároveň princip, jak je možné myslet a pracovat s významem jinak. Zároveň se myší stává kazdy, kdo systém používá, protože je mu umožněno videt jako Myš, která vidí všechno nejen z jednoho, ale z několika úhlů pohledu - usporadanych do kruhu viz topology.json. Pro myš není nikdy význam jen jeden, neuzavírá se a myš nesoudí a nehodnotí, pro ni dobré a špatné nezávisí na společenské dohodě nebo běžném přístupu, ale pouze na souvislostech a kontextu. Myš totiž vidí jednu a tu stejnou věc v každém kontextu znamenající věc jinou a také ví to, že význam může být měřenou a počitatelnou veličinou. Ironie, absurdita, generativita, halucinace a hlavne abstrakce a jejich vznik a chování se dá přesně změřit a to bez kontextu pouze ze struktury.
- System odděluje sémantiku od struktury a počítá s tím, že sémantický význam by měl být do struktury přinesen zvenčí, uživatelem a ne vnucován zevnitř jako pravda či dogma. Významová neuzavřennost je jeho  základní vlastnost pro funkčnost.
